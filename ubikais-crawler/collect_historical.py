...
def main():
    print("=" * 70)
    print("역사 NOTAM 데이터 수집 (2015-2022)")
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        for month in range(1, 13):
            ...
            while current <= month_end:
                ...
                # 배치가 쌓이면 Supabase에 업로드
                if batch:
                    uploaded = upload_to_supabase(batch)
                    year_uploaded += uploaded

                print(f"    {from_date} ~ {to_date}: {len(records)}건 → {len(batch)}건 신규")
        ...
        print(f"\n  {year}년 총계: {year_count}건 (업로드: {year_uploaded}건)")
    ...
    output_file = os.path.join(OUTPUT_DIR, f'historical_notams_{timestamp}.json')
