...
/**
 * 절차 색상 - Navigraph Charts 스타일
 */
export const PROCEDURE_COLORS = {
  SID: '#cb4c8b',      // 핑크/마젠타 계열
  STAR: '#6ca550',     // 녹색 계열
  APP: '#ec7b2c',      // 주황색 계열
  APPROACH: '#ec7b2c', // APP와 동일
  AIRPORT: '#198080',  // 청록색 계열
  AIRWAY: '#7e429a'    // 보라색 계열
};

/**
 * 절차 색상 생성 (Navigraph Charts 기반)
 */
export const getProcedureColor = (type: string, index: number): string => {
  // Navigraph Charts 색상 사용
  if (type === 'SID') return PROCEDURE_COLORS.SID;
  if (type === 'STAR') return PROCEDURE_COLORS.STAR;
  if (type === 'APPROACH' || type === 'APP') return PROCEDURE_COLORS.APP;
  if (type === 'AIRPORT') return PROCEDURE_COLORS.AIRPORT;
  if (type === 'AIRWAY') return PROCEDURE_COLORS.AIRWAY;
