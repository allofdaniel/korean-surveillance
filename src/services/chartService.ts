...
export async function getAvailableVersions(): Promise<string[]> {
  try {
    ...
    const versions = (payload as { data?: string[] }).data;
...
export async function getChartIndex(version?: string): Promise<ChartIndex | null> {
  ...
  try {
    const url = version
      ? `${CHARTS_BASE_URL}/index?version=${encodeURIComponent(version)}`
    ...
    const payload = await response.json() as { data?: ChartIndex };
    const index = payload.data || (payload as unknown as ChartIndex);
...
export async function getAirportCharts(
  ...
  try {
    ...
    if (chartTypes && chartTypes.length > 0) {
      ...
      url += `${separator}type=${chartTypes.join(',')}`;
    }

    const response = await fetch(url);
    if (response.ok) {
      const result = await response.json() as { data?: { charts: ChartData[] } };
