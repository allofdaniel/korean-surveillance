...
const useGlobalLayers = (
  ...
  type LayerFeatureEvent = mapboxgl.MapMouseEvent & { features?: mapboxgl.GeoJSONFeature[] };
  type LayerMouseEvent = (event: LayerFeatureEvent) => void;
  ...
  useEffect(() => {
    ...
    // ---- Waypoints (Navigraph Charts 스타일 적용) ----
    if (showWaypoints && globalData.waypoints) {
      ...
      m.addLayer({
        id: 'global-waypoints', type: 'circle', source: 'global-waypoints', minzoom: 7,
        paint: {
    ...
    // ---- Airways ----
      if (showAirways && globalData.airways) {
        ...
        const features = (globalData.airways as Array<Record<string, unknown>>)
          .map((a): AirwayFeature | null => {
            const pts = (a.points as Array<Record<string, unknown>> | undefined) || [];
            const coordinates = pts
              .map((pt) => {
                const lat = typeof pt.lat === 'number' ? pt.lat : null;
                const lon = typeof pt.lon === 'number' ? pt.lon : null;
                return lat === null || lon === null ? null : ([lon, lat] as [number, number]);
              })
            ...
            const airwayType = String(a.type || '');
            return {
              type: 'Feature',
              geometry: {
                type: 'LineString',
              ...
              properties: {
                name: String(a.name || ''),
                type: airwayType === 'R' ? 'RNAV' : 'Conventional',
                color: airwayType === 'R' ? '#00BFFF' : '#FFD700',
                pointCount: coordinates.length,
              },
            };
          })
          .filter((feature): feature is AirwayFeature => feature !== null);
  ...
  useEffect(() => {
    if (!map?.current || !mapLoaded) return;
    const m = map.current;
    const handlers: Array<{ event: LayerEvent; layer: string; fn: (event: mapboxgl.MapMouseEvent) => void }> = [];
    ...
    const on = (layer: string, event: LayerEvent, fn: LayerMouseEvent) => {
      try {
        if (!m.getLayer(layer)) return;
        const listener = (eventData: mapboxgl.MapMouseEvent) => fn(eventData as LayerFeatureEvent);
        m.on(event, layer, listener);
        handlers.push({ event, layer, fn: listener });
      } catch {
        /* */
      }
    };
    ...
    on('global-airports', 'mouseenter', () => setCursor());
    ...
    on('global-navaids', 'mouseenter', () => setCursor());
    ...
    on('global-heliports', 'mouseenter', () => setCursor());
    ...
    on('global-airways', 'mouseenter', () => setCursor());
    ...
    on('global-ctrl-fill', 'mouseenter', () => setCursor());
    ...
    on('global-restr-fill', 'mouseenter', () => setCursor());
    ...
    on('global-fir-fill', 'mouseenter', () => setCursor());
    on('global-fir-fill', 'mouseleave', () => resetCursor());

    return () => {
      handlers.forEach(({ event, layer, fn }) => { try { m.off(event, layer, fn); } catch { /* */ } });
      removePopup();
    };
  }, [
    map, mapLoaded,
