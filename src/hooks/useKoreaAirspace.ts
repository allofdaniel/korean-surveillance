...
const useKoreaAirspace = (
  ...
  useEffect(() => {
    ...
    ['korea-routes', 'korea-routes-3d', 'korea-routes-labels', 'korea-waypoints', 'korea-waypoint-labels', 'korea-navaids', 'korea-navaid-labels',
     ...
     'korea-sids', 'korea-sids-labels', 'korea-stars', 'korea-stars-labels', 'korea-iaps', 'korea-iaps-labels'].forEach(safeRemoveLayer);
    ['korea-routes', 'korea-routes-3d', 'korea-waypoints', 'korea-waypoint-labels-src', 'korea-navaids', 'korea-airspaces',
     'korea-airports', 'korea-runways', 'korea-ils', 'korea-holdings', 'korea-holdings-labels', 'korea-terminal-waypoints',
     'korea-sids', 'korea-stars', 'korea-iaps'].forEach(safeRemoveSource);
    ...
    if (showKoreaWaypoints && waypoints && waypoints.length > 0) {
      ...
      if (is3DView && show3DAltitude) {
        ...
        // Navigraph Charts 스타일 - 줌 레벨 기반 원 크기
        map.current?.addLayer({
          id: 'korea-waypoints',
          type: 'circle',
          source: 'korea-waypoints',
      ...
      if (is3DView && show3DAltitude) {
        ...
        // Navigraph Charts 스타일 - 줌 레벨 기반 라벨 크기
        map.current?.addLayer({
    ...
    if (showKoreaAirports && koreaAirspaceData.airports && koreaAirspaceData.airports.length > 0) {
      ...
      // The coordinates from Navigraph data don't match Mapbox satellite imagery
    ...
    // ========== SID/STAR/IAP Procedures layer ==========
    const procedures = koreaAirspaceData.procedures;
    const procTermWpts = koreaAirspaceData.terminalWaypoints || [];
    const procEnrouteWpts = koreaAirspaceData.waypoints || [];
    const procNavaids = koreaAirspaceData.navaids || [];
    const procAirports = koreaAirspaceData.airports || [];

    // Build waypoint coordinate lookup map
    const wptCoordMap: Map<string, [number, number]> = new Map();

    // Add terminal waypoints
    procTermWpts.forEach(w => {
    ...
    procEnrouteWpts.forEach(w => {
      if (w.name && w.lon && w.lat && !wptCoordMap.has(w.name)) {
        wptCoordMap.set(w.name, [w.lon, w.lat]);
      }
    });

    // Add navaids (VOR, NDB, etc.)
    procNavaids.forEach(n => {
      if (n.ident && n.lon && n.lat && !wptCoordMap.has(n.ident)) {
        wptCoordMap.set(n.ident, [n.lon, n.lat]);
      }
    });

    // Add runway threshold waypoints (RW06L, RW24R, etc.)
    // Also build ILS position map for arc rendering
    const ilsCoordMap: Map<string, [number, number]> = new Map();
    procAirports.forEach(apt => {
      if (!apt.runways) return;
      apt.runways.forEach(rwy => {
        if (!rwy.id || typeof rwy.lat !== 'number' || typeof rwy.lon !== 'number') return;
        // Runway ID format: "06L/24R" - split into both ends
      ...
      // Build ILS coordinate map for AF arc rendering
      if (apt.ils) {
        apt.ils.forEach(ils => {
          if (ils.ident && typeof ils.llz_lon === 'number' && typeof ils.llz_lat === 'number') {
            // Store by full ident and by last character (common reference)
    ...
    // Helper function to interpolate arc points
    // Uses latitude-corrected coordinates for accurate circular arcs
    const interpolateArc = (
      ...
      // Convert to equirectangular projection (correct for latitude)
      // 1 degree lat = 60 NM, 1 degree lon = 60 * cos(lat) NM
      const centerLat = center[1];
      const cosLat = Math.cos(centerLat * Math.PI / 180);

      // Scale longitude to make it equal to latitude in terms of NM
      ...
      // Generate arc points with fixed radius in scaled space
      for (let i = 0; i <= numPoints; i++) {
        const t = i / numPoints;
        const angle = startAngle + (endAngle - startAngle) * t;

        // Calculate point in scaled space
    ...
    // Build navaid coordinate map for arc center detection
    const navaidCoordMap: Map<string, [number, number]> = new Map();
    procNavaids.forEach(nav => {
      if (nav.ident && typeof nav.lon === 'number' && typeof nav.lat === 'number') {
        navaidCoordMap.set(nav.ident, [nav.lon, nav.lat]);
      }
    });

    // Build airport primary navaid map (nearest VOR/DME to airport)
    const airportNavaidMap: Map<string, [number, number]> = new Map();
    procAirports.forEach(apt => {
      // First try ILS
      ...
      // Then try to find nearest VOR/DME
      ...
      procNavaids.forEach(nav => {
        ...
        if (dist < nearestDist && dist < 0.5) { // Within ~30NM
          nearestDist = dist;
    ...
    // Helper function to find arc center
    // For DME arcs, we need to find a navaid that is EQUIDISTANT from both start and end points
    const findArcCenter = (
      startCoord: [number, number],  // Previous waypoint coordinate
      endCoord: [number, number],    // Current waypoint coordinate
      wptName: string,
      airportIcao: string,
      procType: string
    ): [number, number] | null => {
      // Helper to calculate distance in degrees (approximate)
      const calcDist = (a: [number, number], b: [number, number]) =>
        Math.sqrt(Math.pow(a[0] - b[0], 2) + Math.pow(a[1] - b[1], 2));

      // BEST METHOD: Find navaid where distance to start ≈ distance to end
      // This is the mathematical definition of a valid arc center
      let bestCenter: [number, number] | null = null;
      let bestDiff = Infinity;
      const maxDistDiff = 0.015; // ~0.9 NM tolerance for equidistant check

      // Check all navaids for equidistant property
      for (const [, coord] of navaidCoordMap.entries()) {
        ...
        // Must be within reasonable range (5-30 NM typical for DME arcs)
        const avgDist = (distToStart + distToEnd) / 2;
        if (avgDist < 0.05 || avgDist > 0.5) continue; // ~3-30 NM
      ...
      // Also check ILS positions (common for approach procedures)
      for (const [key, coord] of ilsCoordMap.entries()) {
        // Prefer airport's own ILS
      ...
      // FALLBACK: Pattern-based matching from waypoint name
      ...
      // Last resort: airport's primary navaid
      return airportNavaidMap.get(airportIcao) || null;
    };

    // Helper function to render procedure lines
    interface ProcLeg {
      seq: number;
      wpt: string | null;
      path?: string | null;
    ...
    const renderProcedures = (
      ...
      Object.entries(airportProcs).forEach(([procName, legs]) => {
        const coords: [number, number][] = [];
        const sortedLegs = legs.sort((a, b) => a.seq - b.seq);
        ...
        sortedLegs.forEach((leg, idx) => {
          ...
          // Check if this is an AF (Arc to Fix) leg
          if (leg.path === 'AF' && idx > 0 && coords.length > 0) {
            const prevCoord = coords[coords.length - 1] as [number, number];
            // Determine procedure type from layer ID
            const procType = layerId.includes('sids') ? 'sids' :
                           layerId.includes('stars') ? 'stars' : 'iaps';
            // Pass both start (prevCoord) and end (coord) to find equidistant navaid
            const arcCenter = findArcCenter(prevCoord, coord, leg.wpt || '', airportIcao, procType);

            if (arcCenter) {
              // Interpolate arc points with more points for smoother arcs
              const arcPoints = interpolateArc(
                prevCoord,
                coord,
                arcCenter,
                leg.turn || 'R',
                20  // More points for smoother arc
              );
              // Add arc points (skip first as it's the previous point)
              arcPoints.slice(1).forEach(pt => coords.push(pt));
            } else {
              // Fallback to straight line if no arc center found
              coords.push(coord);
            }
          } else {
            coords.push(coord);
        ...
        if (coords.length >= 2) {
          features.push({
            ...
            properties: { name: procName, airport: airportIcao, type: layerId.replace('korea-', '').toUpperCase() }
          });
        }
      });
      ...
      if (features.length > 0) {
        ...
        // Determine line style based on procedure type
        const isSid = layerId.includes('sids');
        const isStar = layerId.includes('stars');
        const dashArray = isSid ? [1] : isStar ? [4, 2] : [2, 1]; // SID solid, STAR dashed, IAP dotted
    ...
    // Render SIDs
    if (showKoreaSids && procedures?.sids && selectedKoreaAirport) {
      renderProcedures(procedures.sids, selectedKoreaAirport, 'korea-sids', '#4CAF50'); // Green
    }

    // Render STARs
    if (showKoreaStars && procedures?.stars && selectedKoreaAirport) {
      renderProcedures(procedures.stars, selectedKoreaAirport, 'korea-stars', '#2196F3'); // Blue
    }

    // Render IAPs
    if (showKoreaIaps && procedures?.iaps && selectedKoreaAirport) {
      renderProcedures(procedures.iaps, selectedKoreaAirport, 'korea-iaps', '#FF9800'); // Orange
    }
  }, [map, mapLoaded, koreaAirspaceData, showKoreaRoutes, showKoreaWaypoints, showKoreaNavaids, showKoreaAirspaces, showKoreaAirports, is3DView, show3DAltitude, showKoreaHoldings, showKoreaTerminalWaypoints, showKoreaSids, showKoreaStars, showKoreaIaps, selectedKoreaAirport]);
  ...
  }, [map, mapLoaded, showKoreaRoutes, showKoreaWaypoints, showKoreaNavaids, showKoreaAirspaces, showKoreaAirports, showKoreaHoldings, showKoreaTerminalWaypoints, isDayMode]);
};
